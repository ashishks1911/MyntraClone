...
const RemoveSelectedItemDialog = ({ setRemoveSelectedDialog }) => {
  const bagItems = useSelector((store) => store.bag);
  const selectedItems = bagItems.filter((item)=>item.checked);
  const dispatch = useDispatch();
  const handleRemove = ()=> {
    dispatch(bagActions.removeNotSelected());
    setRemoveSelectedDialog(false);
  }
        ...
        <div className=' bg-white w-[22rem] p-2'>
          <div className='pb-5 border-b'>
            <div className='flex justify-between p-1'>
              <div className='p-2'>
                <h3 className='font-bold'>Remove {selectedItems.length} items</h3>
                <p className='text-[14px]'>Are you sure you want to remove {selectedItems.length} items from bag ?</p>
              </div>
              <div className='cursor-pointer' onClick={() => setRemoveSelectedDialog(false)} >
          ...
          <div className='flex text-sm px-4 py-2'>
            <button className='font-bold flex-1 uppercase' onClick={handleRemove}>Remove</button>
...
export default RemoveSelectedItemDialog
